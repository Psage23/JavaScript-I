...
let carModels = function() {
  ...
  carModelslist.push(inventory[i] ['car_model'])
  };
  return carModelslist.sort()
};
//let carModelsSorted = [];
...
let carYears = function() {
  let carYearslist = []
  for (let i = 0; i < inventory.length; i++) {
  carYearslist.push(inventory[i] ['car_year'])
  };
  return carYearslist
  };
...
let BMWAndAudi = inventory.filter(expensiveCar => expensiveCar.car_make === 'BMW' || expensiveCar.car_make ==='Audi')
